...
import { WsVerticalNavigationAsideItemComponent } from '@ws/components/navigation/vertical/components/aside';
...
export class WsVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
  ...
  // 导航模式更改后发出的事件
  @Output() readonly modeChanged: EventEmitter<WsVerticalNavigationMode> = new EventEmitter<WsVerticalNavigationMode>();
  // 导航的打开状态更改后发出的事件
  ...
  // 基本导航菜单拖动后发出的事件
  @Output() readonly basicItemDropChanged: EventEmitter<CdkDragDrop<WsNavigationItem[]>> = new EventEmitter<
    CdkDragDrop<WsNavigationItem[]>
  >();
  @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

  activeAsideItemId: string | null = null;
  onCollapsableItemCollapsed: ReplaySubject<WsNavigationItem> = new ReplaySubject<WsNavigationItem>(1);
  onCollapsableItemExpanded: ReplaySubject<WsNavigationItem> = new ReplaySubject<WsNavigationItem>(1);
  onRefreshed: ReplaySubject<boolean> = new ReplaySubject<boolean>(1);

  private _animationsEnabled: boolean = false;
  private _asideOverlay: HTMLElement;
  private readonly _handleAsideOverlayClick: SafeAny;
  private readonly _handleOverlayClick: SafeAny;
  ...
  constructor(
    ...
    this._handleAsideOverlayClick = (): void => this.closeAside();
    this._handleOverlayClick = (): void => this.close();
  }
   ...
   * 组件内联样式的宿主绑定
   */
  @HostBinding('style') get styleList(): SafeAny {
    return {
  ...
  set wsScrollbarDirectives(wsScrollbarDirectives: QueryList<WsScrollbarDirective>) {
    ...
    // 更新可折叠项的滚动条
    this._wsScrollbarDirectivesSubscription = merge(this.onCollapsableItemCollapsed, this.onCollapsableItemExpanded)
  ...
  ngAfterViewInit(): void {
    ...
    setTimeout(() => {
      ...
      if (!this._navigationContentEl.nativeElement.classList.contains('ps')) {
        // 找到活动项目
        const activeItem = this._navigationContentEl.nativeElement.querySelector('.ws-vertical-navigation-item-active');
      ...
      // 否则
        ...
        this._wsScrollbarDirectives.forEach(wsScrollbarDirective => {
          ...
          // 滚动到活动元素
          wsScrollbarDirective.scrollToElement('.ws-vertical-navigation-item-active', -120, true);
        });
      }
    });
  }
   ...
   * @param event
   */
  basicItemDrop(event: CdkDragDrop<WsNavigationItem[]>): void {
