...
export function WsAnimation(
  ...
  options?: Record<string, SafeAny> | null,
): AnimationTransitionMetadata {
  const duration = stateChange === ':enter' ? WsAnimationDurations.entering : WsAnimationDurations.exiting;
  const curves = stateChange === ':enter' ? WsAnimationCurves.deceleration : WsAnimationCurves.acceleration;
