...
export class GridProvider {
  ...
  private _batchAddButton: ItemModel;
  private _enableButton: ItemModel;
  private _disableButton: ItemModel;
  ...
  constructor(
    ...
    setTimeout(() => (this._authService = this.injector.get(AuthService)));

    this._batchAddButton = {
      text: '批量新增',
      tooltipText: '批量新增',
      prefixIcon: 'e-group-2 text-green-500',
      id: 'batchAdd',
    };
    ...
    this.toolbarWithBatchAdd.splice(1, 0, this._batchAddButton);

    this.toolbarWithSwitch.splice(3, 0, { type: 'Separator' });
    this.toolbarWithSwitch.splice(4, 0, this._enableButton);
    this.toolbarWithSwitch.splice(5, 0, this._disableButton);

    this.toolbarAll.splice(1, 0, this._batchAddButton);
    this.toolbarAll.splice(4, 0, { type: 'Separator' });
    this.toolbarAll.splice(5, 0, this._enableButton);
    this.toolbarAll.splice(6, 0, this._disableButton);
  }
  ...
  /** 工具栏 (全部) */
  readonly toolbarAll = cloneDeep(this.toolbar);
   ...
   * @param args
   */
  created(args: object, gridInstance: GridComponent): void {
    // 工具栏处理
    this.toolbarHandle(gridInstance, 'text');
  }
   ...
   * @param args
   */
  dataBound(args: object, gridInstance: GridComponent): void {
    // 工具栏处理
    this.toolbarHandle(gridInstance, 'isEnable');
   ...
   * @param gridInstance
   */
  defaultHandle(gridInstance: GridComponent): void {
    ...
    /** 分组 */
    gridInstance.allowGrouping = false;
    gridInstance.groupSettings = this.groupSettings;
    ...
    /** 标题在滚动中可见 */
    gridInstance.enableStickyHeader = true;
   ...
   * @param args
   */
  defaultToolbarClick(args: ClickEventArgs, gridId: string, gridInstance: GridComponent): void {
    switch (args.item.id) {
      // Excel 导出
   ...
   * @param type
   */
  toolbarHandle(gridInstance: GridComponent, type: 'text' | 'isEnable'): void {
    const toolbarModule = gridInstance.toolbarModule;
    ...
    switch (type) {
      ...
      case 'isEnable':
        // 工具栏按钮启用禁用处理
        const rowsObject = gridInstance.getRowsObject();
        for (const tbar of tbarEle) {
          if (!tbar.classList.contains('e-separator')) {
            setStyleAttribute(tbar, { background: 'var(--ws-bg-card)' });
   ...
   * @param gridInstance
   */
  pageResizeAuto(gridInstance: GridComponent): void {
    setTimeout(() => {
      // 行高
      const rowHeight = gridInstance.getRowHeight() - 2;
      const gridcontent = gridInstance.element.querySelector('.e-gridcontent .e-content');
      // 聚合计算
      const gridfooter = gridInstance.element.querySelector('.e-gridfooter');
      const footerHeight = (gridfooter && gridfooter.clientHeight) ?? 0;
      // 分页
      const gridpager = gridInstance.element.querySelector('.e-gridpager');
      const pagerHeight = (gridpager && gridpager.clientHeight + 2) ?? 0;
      // 计算剩余行数并向下取余
      const contentHeight = gridcontent.clientHeight - footerHeight - pagerHeight;
      const pageSize = this.pageSettings.pageSize;
      const pageResize = (contentHeight - pageSize * rowHeight) / rowHeight;
      gridInstance.pageSettings.pageSize = pageSize + Math.floor(pageResize);
    });
  }
}
