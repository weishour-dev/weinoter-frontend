...
export class DevUIThemeService {
  ...
  genThemeData(colorChanges: Array<IColorDef>, isDark = false, effect?: IEffect): IThemeData {
  ...
  private fillEmptyColor(pattern: IColorHierarchy, effect: IEffect): void {
  ...
  private getColorEffectOffset(source: Color, colorOffset: IColorOffset, effect?: IEffect): SafeAny {
    let result;
    switch (effect) {
      case 'hsl':
        result = {
  ...
  private getColorValue(source: Color, offset: ISingleOffset, mode: 'hsl' | 'hsv'): Color {
    const sourceModel = source[mode]();
    const key = mode.split('');
    const value = {
  ...
  private getHexOrRgba(color: Color): string {
    if (color.alpha() < 1) {
      const rgb = color.rgb();
  ...
  private minmax(v: number, min: number, max: number): number {
    if (v < min) {
      return min;
    }
    if (v > max) {
