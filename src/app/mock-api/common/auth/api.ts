...
export class AuthMockApi {
  private readonly _secret: SafeAny;
  private _user: SafeAny = {};
  ...
  registerHandlers(): void {
    ...
    // ----------------------------------------------------------------------------
    this._wsMockApiService.onPost('api/auth/forgot-password', 1000).reply(() => [200, true]);

    // ----------------------------------------------------------------------------
    // @ Reset password - POST
    // ----------------------------------------------------------------------------
    this._wsMockApiService.onPost('api/auth/reset-password', 1000).reply(() => [200, true]);

    // ----------------------------------------------------------------------------
    // @ Sign in - POST
    // ----------------------------------------------------------------------------
    this._wsMockApiService.onPost('api/auth/login', 1500).reply(({ request }) => {
      // Sign in successful
      if (request.body.username === 'admin@weishour.com' && request.body.password === '123456') {
        return [
          200,
          {
    ...
    // ----------------------------------------------------------------------------
    this._wsMockApiService.onPost('api/auth/refresh-access-token').reply(({ request }) => {
    ...
    // @ Sign up - POST
    // ----------------------------------------------------------------------------
    this._wsMockApiService.onPost('api/auth/sign-up', 1500).reply(() =>
      // Simply return true
      [200, true],
    );
    ...
    // @ Unlock session - POST
    // ----------------------------------------------------------------------------
    this._wsMockApiService.onPost('api/auth/unlock-session', 1500).reply(({ request }) => {
      // Sign in successful
      if (request.body.username === 'admin@weishour.com' && request.body.password === 'admin') {
        return [
          200,
          {
            user: cloneDeep(this._user),
   ...
   * @private
   */
  private _base64url(source: SafeAny): string {
  ...
  private _generateJWTToken(): string {
    ...
    let signature: SafeAny = encodedHeader + '.' + encodedPayload;
