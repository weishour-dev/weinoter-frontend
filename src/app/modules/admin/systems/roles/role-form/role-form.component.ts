...
export class RoleFormComponent implements OnChanges, OnInit {
  ...
  @ViewChild('rolesCode') rolesCode: TextBoxComponent;
  @ViewChild('rolesName') rolesName: TextBoxComponent;
  ...
  /** 表单数据 (双向绑定) */
  @Input() formData: RoleModel;
  @Output() readonly formDataChange: EventEmitter<RoleModel> = new EventEmitter<RoleModel>();

  /** 列表数据 (双向绑定) */
  @Input() listsData: RoleModel[];
  @Output() readonly listsDataChange: EventEmitter<RoleModel[]> = new EventEmitter<RoleModel[]>();
   ...
   * @returns
   */
  createFormGroup(data: RoleModel): void {
