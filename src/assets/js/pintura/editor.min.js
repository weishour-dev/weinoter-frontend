...
var FilePondPluginImageEditor = (function () {
  ...
  var appendForMeasuring = element => {
    ...
    if (!container.parentNode) document.body.append(container);
  ...
  let isSafari = null;
  var isSafari$1 = () => {
    if (isSafari === null) isSafari = isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
    return isSafari;
  };

  var getImageElementSize = imageElement =>
  ...
  const plugin = _ => {
    ...
    addFilter(
      ...
      (item, { query, dispatch }) =>
        new Promise((resolve, reject) => {
          ...
          // get file reference
          const { file } = item;
          if (!query('GET_ALLOW_IMAGE_EDITOR') || !query('GET_IMAGE_EDITOR_INSTANT_EDIT')) return resolve(item);
    ...
    const createImagePoster = (query, item, done = () => {}) => {
      ...
      renderQueue.queue(next => {
        imageProcessor(file, options).then(imageWriterResult => {
          item.setMetadata('poster', URL.createObjectURL(imageWriterResult.dest), true);
          next();
          done();
          document.dispatchEvent(new CustomEvent('FilePond:poster', { detail: imageWriterResult }));
        });
      });
    };
    ...
    addFilter('CREATE_VIEW', viewAPI => {
      ...
      const openImageEditor = ({ root, props, action }) => {
        ...
        editor.on('load', ({ size }) => {
          ...
          imageState = legacyDataToImageState ? legacyDataToImageState(editor, size, imageState) : imageState;
        ...
        editor.on('process', ({ dest, imageState, src }) => {
          ...
          document.dispatchEvent(new CustomEvent('FilePond:process', { detail: imageState }));
      ...
      const didLoadItem = ({ root, props }) => {
        ...
        if (!query('GET_IMAGE_EDITOR_ALLOW_EDIT') || !query('GET_IMAGE_EDITOR_SUPPORT_EDIT')) return;
        ...
        new Promise((resolve, reject) => {
          ...
          const prepare = () => {
            ...
            renderQueue.queue(next => {
              ...
              if (!imageProcessor || !imageReader || !imageWriter || !editorOptions) return;
              const [createImageReader, imageReaderOptions] = imageReader;
              const [createImageWriter = noop, imageWriterOptions] = imageWriter;
              imageProcessor(file, {
                ...editorOptions,
    ...
    return {
      options: {
        ...
        imageEditorSupportEdit: [isBrowser() && isModernBrowser() && supportsWebGL(), Type.BOOLEAN],
